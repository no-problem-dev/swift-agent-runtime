...
`swift-agent-runtime` は 2 つの責務をターゲットで分離して提供する。

- **`AgentLoopKit`** — A2A 非依存の汎用エージェントループ。`swift-llm-client` と `swift-acp`（ACPCore）に依存し、LLM 呼び出し・ツール並列実行・`input-required` 中断・ACP `session/update` 射影を担う
